...
标签工作流分别交付原生 Edge 二进制与 systemd 物料、`yufeng-modelside` Python wheel 与 systemd 物料、Compose 部署包，以及 Linux amd64 的 Edge 和 ModelSide 容器镜像归档。容器目标主机可以先校验 `yufeng-v0.2.0-checksums.txt`，再显式载入镜像：

```sh
gzip -dc yufeng-v0.2.0-edge-image-linux-amd64.tar.gz | docker load
gzip -dc yufeng-v0.2.0-modelside-image-linux-amd64.tar.gz | docker load
export YUFENG_EDGE_IMAGE=yufeng-edge:v0.2.0
export YUFENG_MODELSIDE_IMAGE=yufeng-modelside:v0.2.0
```
...
`scripts/preflight-release-evidence.sh` 在合入 `develop` 前对预期合并 Git 树运行一次完整本机静态清单，包括竞态、构建标签、格式化、综合静态分析、漏洞、Protocol Buffers 消息契约、控制台、交叉编译和热路径基准。合入后 `scripts/archive-live-evidence.sh` 只补一次活栈、恢复和容量结果，不重跑静态清单；它必须覆盖旁路关闭、ModelSide 空闲、ModelSide 满载、Brain 断连和磁盘变慢下每秒 2000 个请求及第 99 百分位延迟预算。软件证据不能替代客户现场的代理网段、上游、证书、网络核对和变更责任记录。
