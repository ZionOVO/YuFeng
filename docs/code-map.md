...
| 检测器实现 | 规则检测器：`lib/edgecore/rule.go`；Coraza DetectionOnly：`lib/edgecore/coraza.go`（CRS 4.25.0）；规范化视图与覆盖度已建。**画像核未装载** | 部分已建 |
| 初次配置引导 | 契约 `docs/api.md` §19；协议 `proto/yufeng/onboarding/v1`（含 `OnboardingGate` 与 `PutDeploymentSpecification`）；库表 `deployment_onboarding` 与 `credential_slots`；服务 `lib/brain/onboarding_service.go`、`onboarding_deployment_specification.go` | 六步状态机、配置写入、模型探测、部署规格确定性签发、Edge 主动心跳就绪和完成谓词已装配；不创建 Edge 进程或容器 |
...
| 企业试点交付证据 | 合并前静态预检 `scripts/preflight-release-evidence.sh`；静态与活栈入口 `scripts/delivery-evidence.sh`；合并后只补活栈并装配 `scripts/archive-live-evidence.sh`；环境指纹 `scripts/release-environment.py`；秘密扫描与远端复核 `scripts/release-evidence.py`；发布元数据校验 `scripts/release-metadata.py`；双阶段发布 `.github/workflows/release.yml`；历史与格式说明 `docs/delivery-evidence.md` | 静态预检绑定预期合并 Git 树；最终 `develop` 只确认谱系并补一次活栈，归档不重跑静态。当前版本只有在 GitHub Release 非草稿、平台制品与三项证据资产齐全且摘要一致后才算发布；客户现场变更单仍须由现场负责人关闭 |
...
| PostgreSQL 连接池 / goose 迁移 / sqlc | `lib/store/store.go`、`lib/store/traffic_role.go`、`lib/store/migrations/`、`lib/store/sqlc/` | 治理池、最大四连接的隔离流量池与 45 个迁移已建；生产启动用 `ValidateRestrictedTrafficRole` 确认 `yufeng_traffic` 只有规定流量表的 `SELECT` / `INSERT` 且不能访问治理表；两个池仍在同一中台进程；sqlc 生成层无业务调用方 |
