...
2. `VERSION`、`console/package.json`、`console/package-lock.json`、`components/modelside/pyproject.toml` 与 `components/modelside/yufeng_modelside/__init__.py` 声明同一版本；
...
单个时间敏感测试首次失败时，若同一提交的拉取请求门禁已经通过该测试，允许对同一提交重跑一次完整 Go 门禁。只有重跑的 `make build test vet` 全部成功才可继续；重复失败、多个测试失败，以及构建、静态分析、网络契约或制品检查失败均不得通过重跑容忍。正常成功路径不追加重复测试。
...
- `yufeng-v0.1.1-linux-mips.tar.gz`
- `yufeng-v0.1.1-windows-amd64.tar.gz`
- `yufeng-v0.1.1-darwin-amd64.tar.gz`
- `yufeng-v0.1.1-darwin-arm64.tar.gz`
- `yufeng-v0.1.1-console.tar.gz`
- `yufeng-v0.1.1-modelside-python.tar.gz`
- `yufeng-v0.1.1-deployment.tar.gz`
- `yufeng-v0.1.1-edge-image-linux-amd64.tar.gz`
...
持续集成、开发平台复验和发布工作流记录实际运行耗时，但仓库不设置作业时长门禁，也不因构建较慢判定失败。测试内部用于识别死锁、协议超时或存活时限的行为约束，以及部署验收中的容量和延迟指标，不属于构建耗时门禁。
