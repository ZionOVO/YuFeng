...
- **模型网关**：大语言模型唯一出网口，挂在 **brain 进程内**（[定义](glossary.md#modelgateway)）。生产默认 `base_url=https://api.x.ai/v1`、方言 `MODEL_DIALECT_OPENAI_CHAT`、`model` 缺省 `DefaultChatModel`（引导里可改）；操作方可以配置绝对 HTTP 或 HTTPS 地址，供应商密钥是中台凭据槽中的可选项，未配置时不得发送认证头。同时一条槽。敏感证据生成仍只允许 HTTPS。出网按槽上的[模型方言](glossary.md#model-dialect)转发 OpenAI Chat、OpenAI Responses 或 Claude Messages（`docs/api.md` §19.4）；上游不可达、非成功状态、非法响应或空文本一律明确失败，不得合成兜底回答。引导完成后管理员可在控制台 `/app/model` 改槽并看窗内成功率，不退引导状态。确定性模型剧本只存在于测试编译单元，不进入交付二进制、Compose 活栈或交付验收；`-dev-insecure` 只放宽本地传输，不启用固定回答。贾维斯生产**禁止** `-model-url` 旗标（含指向 brain 或内网）。流量深度学习不经过本模型网关，也不在 brain 进程执行；生产贾维斯走带账本、租约与 ModelAttempt 的 `Generate`，`CompleteChat` 只保留迁移兼容和槽连通性探测（`docs/api.md` §18.10）。Google Agent Development Kit 只借形状，不进进程（ADR-003 / ADR-026）。
...
| 签名检测 | 自维护 Coraza `v3.7.0-zion.1`（发布提交 `f5daf73dd76069d1dd510cf95df10f16d2ac7ac7`，内含自维护 libinjection-go `v0.3.3-zion.1`）+ 开放全球应用安全项目核心规则集 4.25.0（永远 DetectionOnly） | 保留 `github.com/corazawaf/coraza/v3` 导入路径，通过 `go.mod` 固定替换到 `github.com/ZionOVO/coraza/v3@v3.7.0-zion.1`；官方 v3.7.0 只作 `SecRxPreFilter Off` 对照基线 | 已冻结并引入；`SecRxPreFilter On`（实验性），引擎命中本身不 403 |
...
**Coraza 平行仓维护（架构决策记录 041）**：御锋保留上游模块导入路径，只以远端不可变发布标签替换到自维护仓，不使用本地目录或浮动分支。自维护仓先用独立拉取请求同步上游，再用独立拉取请求承载性能修改；依赖 fork 必须先发布并通过代理与校验和解析，再更新御锋的固定版本。御锋只有在检测键差分、核心规则集回归、预筛选安全测试、模糊测试、构建和同口径性能基准全部通过后才更新发布标签。当前生产规则显式启用实验性的 `SecRxPreFilter On`；官方 v3.7.0 的 `Off` 黄金数据和历史 `Off` 性能数据只能作为对照，不能与开启预筛选的数据混写。规则集版本、装载范围、`DetectionOnly` 和 Gate 语义不因替换引擎来源而改变。
...
| 041 | Coraza 由固定远端发布标签的自维护平行仓提供，保留上游模块导入路径 | 上游同步、性能修改与御锋依赖更新分别评审；依赖先发布，检测等价、预筛选安全性、模糊测试和同口径基准同时约束升级，官方 `Off` 对照与生产 `On` 实验数据分开记录，避免本地替换或实验配置成为不可复现的生产依赖 |
