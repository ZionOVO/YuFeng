...
- [x] **同时交付 Edge 原生二进制和独立容器镜像。**
  ...
  - 结果：Darwin arm64 本机已分别构建 Linux arm64 与发布工作流实际归档的 Linux amd64 镜像；两者入口冒烟成功，镜像标签为 `v0.2.0`，运行用户为 `65532:65532`。
...
- [x] **交付独立 Python `yufeng-modelside` 服务包与镜像。**
  ...
  - 结果：6 项 Python 合同、采样与断连测试通过，`yufeng_modelside-0.2.0-py3-none-any.whl` 构建成功；Linux arm64 和 Linux amd64 TensorFlow 镜像均完成构建与入口冒烟。依赖使用跨平台 `tensorflow` 发行包，并以源码合同测试禁止回退到缺少 Linux arm64 wheel 的 `tensorflow-cpu`。
...
- [x] **缩短无价值的 Go 测试耗时并保持行为覆盖。**
  - 定位：`go test -json ./...` 与包级耗时报告；慢测试对应源码和语义断言；`.github/workflows/pull-request.yml`、`ci.yml`。
...
- [ ] **用发布稳定分支完成一次预检和一次 develop 合并。**
  - 定位：`docs/architecture.md` 架构决策记录 037；`docs/delivery-evidence.md` 第 3、4 节；`scripts/preflight-release-evidence.sh`；`scripts/archive-live-evidence.sh`；`.github/workflows/pull-request.yml`、`ci.yml` 与 `release-gate.yml`；发布分支和拉取请求记录。
  - 验收：从冻结的 `origin/develop` 创建 `release/v0.2.0`；发布问题只在该分支集中修复；对预期合并 Git 树本机完整运行一次竞态、构建标签、格式化、综合静态分析、漏洞、消息契约、控制台、交叉编译和热路径基准，不重复普通测试或被竞态包含的定向 Go 脚本；远端快速门禁确认后以合并提交只进入 `develop` 一次；最终 `develop` 持续集成只确认精确提交和拉取请求谱系；证据提升验证 Git 树、两个父提交、环境指纹和有效期，只补一次活栈、恢复和五场景性能，再用已成功的持续集成 URL 归档且不重跑静态套件。私有仓库计划无法强制分支保护时仍由合并人等待成功证据，不描述成平台锁。
